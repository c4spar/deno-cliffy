...
export class BaseCommand<O = any, A extends Array<any> = any> {
  ...
  public getGlobalCompletions(): ICompleteSettings[] {
    const getCompletions = (
      cmd: BaseCommand | undefined,
      completions: ICompleteSettings[] = [],
      names: string[] = [],
    ): ICompleteSettings[] => {
  ...
  public getGlobalCompletion(name: string): ICompleteSettings | undefined {
    ...
    let completion: ICompleteSettings | undefined = this._parent
  ...
  public option(
    ...
    const args: IArgumentDetails[] = result.typeDefinition
  ...
  public env(name: string, description: string, options?: IEnvVarOption): this {
    ...
    const details: IArgumentDetails[] = this.parseArgsDefinition(
  ...
  protected parseType(
    ...
    return type.handler instanceof Type
      ? type.handler.parse(option, arg, nextValue)
      : type.handler(option, arg, nextValue);
  }
    ...
    if (!this.hasArguments()) {
      if (args.length) {
        if (this.hasCommands(true)) {
          ...
          throw this.error(
            new Error(`No arguments allowed for command: ${this._name}`),
          );
        }
      }
    } else {
  ...
  protected parseArgsDefinition(argsDefinition: string): IArgumentDetails[] {
    ...
    for (const arg of parts) {
      ...
      const type: OptionType | string | undefined = parts[2]
        ? parts[2]
        : OptionType.STRING;
  ...
  public getGlobalCommand<O = any>(
    ...
    let cmd: BaseCommand | undefined = this._parent.getBaseCommand(
      name,
      hidden,
    );
  ...
  public getBaseEnvVar(
    name: string,
    hidden?: boolean,
  ): IEnvVariable | undefined {
    const envVar: IEnvVariable | undefined = this.envVars.find((env) =>
  ...
  public getGlobalEnvVar(
    ...
    let envVar: IEnvVariable | undefined = this._parent.getBaseEnvVar(
      name,
      hidden,
    );
  ...
  public writeError(...args: any[]) {
    stderr.writeSync(
      encode(fill(2) + red(format(`[ERROR:${this._name}]`, ...args))),
    );
  }
  ...
  public error(error: Error, showHelp: boolean = true): Error {
    ...
    const CLIFFY_DEBUG: boolean = hasEnvPermissions
