...
export class CompleteCommand extends DefaultCommand {
  public constructor(protected parent: BaseCommand) {
    ...
    this.arguments("<action:action> [command...:command]")
      ...
      .action(
        async (options: IFlags, action: string, commandNames: string[]) => {
          let cmd: BaseCommand | undefined = commandNames
            .reduce(
              (
                cmd: BaseCommand | undefined,
                name: string,
              ): BaseCommand | undefined => cmd?.getCommand(name, false),
              parent,
            );
          ...
          const completion: ICompleteSettings | undefined = cmd.getCompletion(
            action,
          );
