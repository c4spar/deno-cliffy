...
`.trim();
  ...
  private generateCompletions(command: BaseCommand, root?: boolean): string {
    if (
      !command.hasCommands(false) && !command.hasOptions() &&
...
function _${snakeCase(command.getPath())}() {` +
  ...
  protected generateCommandCompletions(command: BaseCommand): string {
    ...
    if (command.hasArguments()) {
      const completionsPath: string = command.getPath().split(" ").slice(1)
        .join(" ");
    ...
    if (command.hasCommands(false)) {
      const actions: string = command
        ...
        .map((command: BaseCommand) =>
          `${command.getName()}) _${snakeCase(command.getPath())} ;;`
        )
        .join("\n            ");
  ...
  protected generateOptions(command: BaseCommand) {
    ...
    for (const option of command.getOptions(false)) {
      ...
      options.push(
        this.generateOption(option, baseName, completionsPath, flagExcluded),
      );
    }
