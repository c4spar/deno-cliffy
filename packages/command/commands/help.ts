...
export class HelpCommand extends BaseCommand implements IHelpCommand {
  ...
  public getHelp(name?: string): string {
    const cmd: BaseCommand | undefined = name
      ? this.parent.getCommand(name, false)
      : this.parent;
    ...
    const renderLine = (...args: any[]) =>
      output += (args.length ? " ".repeat(indent) + format(...args) : "") +
        "\n";
    ...
    const getHints = (option: IFlagOptions): string => {
      ...
      typeof option.default !== "undefined" &&
        hints.push(blue(bold(`Default: `)) + blue(format(option.default)));
