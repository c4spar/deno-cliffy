...
export class Table extends Array<IRow> {
  ...
  protected calc(): ICalc {
    ...
    for (let colIndex: number = 0; colIndex < columns; colIndex++) {
      let minCellWidth: number = Array.isArray(this.options.minCellWidth)
        ? this.options.minCellWidth[colIndex]
        : (typeof this.options.minCellWidth === "undefined"
          ? MIN_CELL_WIDTH
          : this.options.minCellWidth);

      let maxCellWidth: number = Array.isArray(this.options.maxCellWidth)
  ...
  protected renderBorderTopRow(
    ...
    for (let i = 0; i < opts.columns; i++) {
      const cell: Cell = row[i];
      cells.push(
        (cell.getBorder() ? border.top : " ").repeat(
          opts.padding[i] + opts.width[i] + opts.padding[i],
        ),
      );
    }

    return " ".repeat(this.options.indent || 0) + border.topLeft +
      cells.join(border.topMid) + (border.topRight) + "\n";
  }

  protected renderBorderMidRow(
    row: Row<Cell>,
    prevRow: Row<Cell> | undefined,
    nextRow: Row<Cell> | undefined,
    opts: ICalc,
    ...
    for (let i = 0; i < opts.columns; i++) {
      ...
      cells.push(
        (cell.getBorder() && nextCell?.getBorder() !== false ? border.mid : " ")
          .repeat(opts.padding[i] + opts.width[i] + opts.padding[i]),
      );
    }

    return " ".repeat(this.options.indent || 0) + border.leftMid +
      cells.join(border.midMid) + (border.rightMid) + "\n";
  }

  protected renderBorderBottomRow(
    row: Row<Cell>,
    prevRow: Row<Cell> | undefined,
    nextRow: Row<Cell> | undefined,
    opts: ICalc,
    ...
    for (let i = 0; i < opts.columns; i++) {
      const cell: Cell = row[i];
      cells.push(
        (cell.getBorder() ? border.bottom : " ").repeat(
          opts.padding[i] + opts.width[i] + opts.padding[i],
        ),
      );
    }

    return " ".repeat(this.options.indent || 0) + border.bottomLeft +
      cells.join(border.bottomMid) + (border.bottomRight) + "\n";
  }
