...
| complete    |             `(input: string, suggestion: string) => string`             |    No    | A custom handler that is called when the input value gets completed with a suggestion. input is the current input value and suggestion is the suggestion that will be used if no other value is returned. |
| fileMode    |                                `boolean`                                |    No    | If enabled, files from the current working directory will be added to the suggestions list.                                                                                                               |
